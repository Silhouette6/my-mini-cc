...
使用 `read_file`、`bash`、`todo_write`、`subagent(worker_type="explore")` 系统化探索陌生代码库。读完入口点后必须用任务看板规划步骤，并积极派子智能体完成各子任务。任务看板在 system prompt 中自动展示，无需单独查询。
...
| 获取方法/函数实现 | get_symbol_body | 优先于 read_file，只返回符号代码省 token；file_path 相对 workdir |
